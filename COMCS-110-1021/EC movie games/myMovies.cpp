...
int ask(string name)//ask question
{
  int ans;
...
int random(int number)//random a number
{
  int a=rand()%number;
  return a;
}
int check(int a, int b)//check the answer
{
  int c;
  if (a==b)
  {
...
char askuser()//ask user to make choice
{
  char choice;
...
char quit1(int a)//ask if user want to add a movie
{
  char choice;
...
void output(deque<Movie>& list)//ouput
{
  int i;
  int j;
...
{
  //Data
  deque<Movie> list;//table 
  int size;	//size of table
  int randomnum;//random number
  int answer;//user answer
  int scores=0;//user score
  int useranswer;//user answer for choice
  string moviename;//adding movie's name
  int movieyear;//adding movie's year
  char choice;// user choice
  int j;//loop
  int i;//loop
  Movie temp;//temp element for table
