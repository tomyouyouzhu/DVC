...
Polynomial Polynomial::operator- (const Polynomial &a) {
	Polynomial z;
	for (int i = 0; i < this->size; i++) {
		z.addTerm(this->getTerm(i).coe, this->getTerm(i).exp);
	}
	for (int i = 0; i < a.size; i++) {
		z.addTerm(0-a.getTerm(i).coe, a.getTerm(i).exp);
	}
	z.size = this->size + a.size;
	for (int i = 0; i < z.size; i++) {
		for (int j = i + 1; j < z.size; j ++) {
			if (z.getTerm(i).exp == z.getTerm(j).exp) {
				z.setTerm(i, z.getTerm(i).coe + z.getTerm(j).coe, z.getTerm(i).exp);
				z.setTerm(j, 0, 0);
			}
		}
	}
...
Polynomial & Polynomial::operator= (const Polynomial &a) {
	if (this != &a) {
		if (this->size != a.size) {
			...
			for (int i = 0; i < this->size; i++) {
				this->setTerm(i, a.getTerm(i).coe, a.getTerm(i).exp);
			}
		}
	}
	return *this;
